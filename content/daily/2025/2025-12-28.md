...
DP：我们在宇树那个实验室里有一个机器人形机器人在那个凳子上坐着。我们在用那个机械臂上面那个机器上的机械臂做那个抓取。嗯，你后面如果要做实验的话。可以用那个来做。

**Unitree G1 EDU**：**Jetson Orin NX + 三指灵巧手 Dex3-1** 是非常理想的实验平台，但实际部署逻辑需要根据模型特性做一些调整。

## Unitree G1 EDU 部署方案

### 1. 硬件算力：Orin NX 能跑动吗？

你提到的 **Jetson Orin NX**（16GB 显存，约 100-157 TOPS）属于高性能边缘算力卡，但面对 7B 或更大的 **VLA** 模型时仍有挑战：

#### π? / OpenVLA

- **问题**：直接运行原版模型会导致显存溢出（**OOM**）或极低帧率（< 1Hz）
- **解决方案**：你需要使用**量化版本（如 INT4/INT8）**或者采用 **Action Chunking（动作分块）**
- **更好选择**：关注 **NanoVLA** 或 Hugging Face 的 **LeRobot** 项目。**Unitree** 官方已经适配了 `unitree_il_lerobot` 框架，而 **LeRobot** 已经原生支持了 $\pi_0$。这意味着你可以直接套用官方提供的转换脚本，把在 **AutoDL** 训练好的模型权重加载到这个框架里，通过 **G1** 的 **SDK** 直接驱动电机，不需要从零开始写驱动

#### ReKep

**ReKep** 的逻辑是"大模型推理 + 实时优化"。它不需要在 **Orin** 上运行整个 **GPT-4o**，而是将摄像头图像发给云端/上位机，上位机返回约束函数，**Orin** 只负责跑本地的**逆运动学（IK）解算**和**低级控制循环**。这种模式非常适合 **Orin NX**。

**ReKep** 的部署更像是"插件式"的：

1. **视觉识别**：在 **PC** 或 **Orin** 上运行一个视觉模型（如 **DINOv2**），识别物体上的关键点
2. **解算约束**：**ReKep** 输出的是 **Python** 定义的约束函数（比如：手指尖和瓶盖的距离为 0）
3. **SDK 落地**：你需要调用 **Unitree SDK2** 的**逆运动学（IK）**接口。你告诉 **SDK** "手要去这个坐标"，**SDK** 会自动帮你算出 **G1** 胳膊和手指的电机该转多少度

### 2. 部署逻辑：AutoDL 训练 → 实机运行

"**AutoDL** 训练，实机部署"是主流工作流，但对于 **G1** 这种人形机器人，中间有几个关键环节：

1. **数据采集（最关键）**：在 **AutoDL** 训练前，你需要先在 **G1** 实机上通过遥操作（**Teleoperation**）采集你那只**三指灵巧手**的操作数据。**Unitree** 提供了 `unitree_il_lerobot` 数据采集工具

2. **模型适配**：
   - **如果用 π?**：你需要将模型最后的 **Action Head（动作头）**修改为适配 **G1** 机械臂（5-7 **DOF**）+ 三指手（7 **DOF**）的维度
   - **如果用 ReKep**：你不需要"训练"大模型，而是需要编写 **G1** 的 `environment.py` 接口，把 **ReKep** 输出的末端位姿（**End-effector Pose**）映射到 **G1** 的 **API** 上

### 3. 这两个模型针对"手"？

你的 **G1** 配备的是 **Dex3-1 三指灵巧手**，这两个模型对它的支持方式不同：

#### π? (Physical Intelligence)

- **它学习的是"动作分布"**。它并不特意针对哪种手，只要你有足够的三指手操作数据喂给它，它就能学会怎么控这只手
- 由于 **π?** 预训练数据中包含大量 **ALOHA**（二指）数据，如果你直接拿来用，它可能无法发挥三指手的灵活性。**你需要进行微调（Fine-tuning）**

#### ReKep (NVIDIA/Stanford)

- **它是"手性无关"的**。**ReKep** 核心在于识别物体上的"关键点"（如瓶盖的中心、杯子的边缘）
- 它会告诉机器人："请让你的夹爪中心靠近瓶盖中心"。至于你是用二指还是三指，**ReKep** 不关心，它只给出一个目标位姿。**这非常适合你开发三指手的复杂捏取动作**

### 4. 针对 Unitree G1 EDU 的实战建议

1. **优先尝试 Unitree 官方框架**：
   - 先跑通 [unitree_il_lerobot](https://github.com/unitreerobotics/unitree_il_lerobot)。这是针对 **G1** 和 **Dex3** 灵巧手专门优化的
   - 它支持 **DP (Diffusion Policy)** 和 **ACT** 算法，这些算法在 **Orin NX** 上的实时性非常出色

2. **ReKep 尝试**：
   - 如果你想做"不需要大量数据、靠视觉推理"的任务，**ReKep** 是上限最高的
   - 你需要一台装有 **RTX 3090/4090** 的 **PC** 作为上位机，通过以太网连接 **G1** 的 **Orin NX**
   - **PC** 跑视觉跟踪和轨迹优化，**Orin** 只负责执行指令

3. **VLA 进阶**：
   - 如果你坚持要跑 **VLA**（如 $\pi_0$），建议先在 **PC** 上跑推理，通过 **ROS2** 把指令发给 **G1**
   - 等效果调好后，再考虑通过 **TensorRT** 将模型压缩部署到 **Orin NX**

## 项目优化思路

参考：[Gemini 对话](https://gemini.google.com/app/da36d586dff55e72)

我暂时还没想好，初步的一个逻辑如下：

### 项目一：$\pi_0$ (VLA) 模型在 G1 灵巧手上的全流程优化与边缘端部署

**优化思路**：重点从"简单微调"转向**"高质量数据工程"与"推理加速优化"**。这能精准击中京东和小鹏 **JD** 中提到的"数据后处理"和"模型部署优化"。

**核心改进点**：

1. **高质量数据引擎 (Data Pipeline)**：
   - 不要只说"用了 **OXE**"，要说你实现了针对 **G1** 三指灵巧手的数据增强算法
   - 例如：将二指夹爪的数据通过**逆运动学 (IK) 重定向 (Retargeting)** 映射到三指手上

2. **引入流匹配 (Flow Matching)**：
   - $\pi_0$ 核心技术是 **Flow Matching** 而非简单的 **Diffusion**
   - 在简历中强调你对流匹配参数的调优经验（针对 **JD 4** 的要求）

3. **真机闭环频率优化**：
   - 针对 **Orin NX** 的显存瓶颈，通过 **TensorRT + INT8 量化**将推理频率从 1Hz 提升至 10Hz+
   - 并实现动作平滑算法（**Lp-Smoothing**），解决端到端模型抖动问题

**简历关键词**：**Flow Matching**、**Action Chunking**、**Cross-embodiment Retargeting**、**TensorRT Deployment**

### 项目二（合订版）：层次化具身智能系统

**由 VLM 驱动、ReKep 引导的长程灵巧操作任务**

**优化思路**：将 **VLM** 的逻辑规划、**ReKep** 的几何约束以及 **Isaac Sim** 的物理仿真融为一体。展示你具备构建"大脑（规划）+ 小脑（控制）"完整系统的能力。

**项目架构**：

1. **任务规划层 (VLM)**：
   - 使用大模型（如 **GPT-4o** 或微调后的 **VLM**）将"清理桌面并把水杯放回原处"等长程指令拆解为多个子任务

2. **视觉约束层 (ReKep)**：
   - 针对每个子任务，通过视觉关键点提取（**DINOv2/SAM**），动态生成 **ReKep** 约束函数
   - **进阶**：针对 **G1** 的三指手，设计了多手指接触力约束，使机器人能完成"拧瓶盖"或"捏取细小零件"等高难度动作

3. **执行层 (WBC & Sim-to-Real)**：
   - 在 **Isaac Sim** 中实现 **G1** 的**全身控制 (Whole-Body Control)**，确保在移动操作中保持平衡
   - 利用**领域随机化 (Domain Randomization)**技术，解决仿真到实机的 gap

**简历关键词**：**Long-horizon Task Planning**、**Visual Keypoints Extraction**、**Constraint-based Optimization**、**Isaac Sim / Gym**、**Zero-shot Generalization**
...
开始了。
MIT 6.S184
(content\docs\self-study\ai\flow-matching\_index.md)
