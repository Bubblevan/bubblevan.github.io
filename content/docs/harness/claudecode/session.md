...
## 9. 走错路以后，为什么最好 Rewind，而不是继续追加“再试试”
...
### 9.4 Rewind 管的不是 Context 大小，而是分支
...
### 10.1 Clear 是扔掉，Rewind 是剪枝，Compact 是压缩
...
### 10.5 所以 `/compact <hint>` 比裸 Compact 更有意思
...
### 11.1 有些 Token 本来就不该进入主 Session
...
### 11.2 Subagent 不是省掉调查，而是改变调查结果的传播范围
...
### 11.3 我以后还需要 Tool Output，还是只需要答案？
...
## 12. 主 Agent 不应该成为垃圾中转站，但 Subagent 也不是免费的
...
### 12.1 如果主 Agent 下一步马上要用，隔离反而可能绕远路
...
### 12.4 所以 Subagent 最值钱的地方是“把过程截断”
...
### 12.8 我真正想优化的是 Intermediate Output 的去向
...
## 13. Shell Output 也是 Context：别等垃圾进来以后才处理
...
### 14.5 “Claude 还记得这个文件吗？”和“再塞一份文件”不是一回事
...
## 15. `/context`：一个新 Session 到底从多少 Token 开始
...
### 15.2 Always-on 和 On-demand 应该分开
...
### 15.5 `/context` 就像给 Session 做一次开机自检
...
### 15.6 `/memory` 和 `/context` 查的是两个不同问题
...
### 15.8 因为这已经开始越过 `session.md` 的边界了
...
## 16. Prompt Cache：为什么长 Session 没有想象中那么贵
...
### 16.4 为什么前面的 Context Lifetime 仍然成立
...
## 17. 为什么中途切 `/model`、`/effort` 会让 Cache 行为改变
...
### 17.1 `/model` 最好理解：每个模型有自己的 Cache
...
### 17.7 但 Fast Mode 的当前实现还有个反直觉细节
...
### 17.8 `/compact` 更有意思：它不是换 Key，而是把 History 改写了
...
### 17.10 甚至 MCP / Plugin 变化也可以用这个逻辑解释
...
### 17.11 所以我现在会把“Session 开局定旋钮”理解成 Cache Hygiene
...
### 17.12 Cheap Moment 和 Expensive Moment
...
下一节就看 Cache TTL。
## 18. 为什么离开键盘之前 `/compact`：时间本身也是成本变量
...
### 18.7 这也解释了为什么 Compact 最适合 Milestone
...
### 18.9 时间让 Context Lifetime 这个词有了第二层意思
...
### 18.10 到这里，Prompt Cache 这一 Macro 才算闭环
...
看钱到底烧在哪里。
## 19. 别再猜 Token 花在哪：我第一次跑了 `ccusage`
...
### 19.1 第一眼最离谱的不是 Cost，而是 Cache Read
...
### 19.2 这也解释了为什么 Total Tokens 看起来会特别吓人
...
### 19.3 我自己的记录里，Cache Read 经常比 Input 大一个数量级
...
### 19.5 更有意思的是：Token 多，不一定 Cost 就高
...
### 19.6 所以我以后看 Usage，第一眼不会再看 Total
...
### 19.8 `ccusage` 更像我的 Agent 仪表盘，而不是优化器
...
### 19.9 但只有 Daily Report 还不能验证前面的工作法
...
## 20. 把感觉变成实验：同一个 Bug 跑三组 Session
...
### 20.5 C 组：整套 Session Discipline
...
### 20.9 `ccusage session` 比 Daily Report 更适合这件事
...
### 20.10 但 Cost 只能写 Estimated Cost
...
### 20.13 如果以后真想知道“到底是哪一项有效”，再做 Ablation
...
> **我自己真的让 Agent 跑过。**
## 21. 最后：Vibe Coding 到底在这张工程地图的哪一层？
...
### 21.1 我以前的 AI-Native 开发流程里，其实漏了一层
...
### 21.3 第二层：Vibe Coding Session Discipline——这一项任务到底怎么一起做？
...
### 21.4 第三层：Harness Engineering——不能每次都靠人手动管
...
### 21.5 第四层：Context Engineering——开始正式设计 Agent 的信息环境
...
### 21.7 所以我现在不太想把所有东西都叫“Vibe Coding”
