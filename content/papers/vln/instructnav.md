...
| 实验任务 | 关键成果 | 性能提升数据 |
|---------|---------|-------------|
| 物体目标导航（ObjNav） | 优于所有零样本方法，接近最优训练模型 OVRL | 较 SOTA 零样本方法提升 10.48% |
| 视觉语言导航（R2R-CE） | 首个实现零样本完成该任务的模型 | 超越多数任务训练模型 |
| 需求驱动导航（DDN） | 大幅领先现有训练方法与零样本方法 | 较 SOTA 提升 86.34% |
| 真实机器人验证 | 成功执行多类型 / 混合类型指令 | 适配 5 类真实场景（公寓、图书馆等） |
